...
const SkillSection: React.FC<SkillSectionProps> = ({ skillsData, theme }) => {
  ...
  return (
    ...
    >
      <div
        className={`absolute top-10 left-1/2 w-full flex justify-center transform -translate-x-1/2 px-4 py-2  rounded-t-xl `}
      >
      ...
      {skillsData[0].skills.map((skill, index) => (
        <div
          key={index}
          className={`skill-item cursor-pointer flex flex-col gap-6 rounded-2xl p-8 border-solid border-[0.25rem]  text-center max-lg:items-center    ${theme === "dark" ? "bg-[--darkblue]" : "bg-[--icewhite]"
            }`}
